...
- 2026-05-26 — **Second-pass test sweep (after user asked to scan for migration/expansion opportunities).** Identified 9 migration candidates and 7 new test slots in `_sessions/75-adopt-timeprovider/test-opportunities.md`. Executed all actionable items except B5 (`Server_DisconnectDelay`) and B6 (`WebSocketPipe_CloseTimeout`) which need raw-TCP harness scaffolding. **Skipped B3** (client-side timeout disconnect): client's own pings race with `FakeTimeProvider.Advance` — every ping fire-and-forget triggers a server pong that resets `LastReceived` under real time. B4 (server watchdog) covers the same architectural property from the safer direction. Net result: 7 pool tests migrated to FakeTimeProvider; auth-cache test hardened; 4 new tests added — ping cadence (B2), relay 500ms reconnect (B7), reconnect-policy delay (B1), server watchdog (B4).
- 2026-05-26 — **Root-cause fix: separate "wait for the kernel" from "app-logic time."** The initial migration over-eagerly routed every `Task.Delay` through `TimeProvider`, including ones that exist purely to give the OS real wall-clock time for real network operations. This made test teardown deadlock under `FakeTimeProvider` because the disconnect path awaited `Task.Delay(DisconnectDelay, fakeTime)` and the test no longer advanced the fake clock. The earlier "fix" was a per-test `DisconnectDelay = 0` workaround that future contributors could forget. The proper fix is to keep network-wait sites on real time. Four sites reverted: `NexusSession.cs:311` DisconnectDelay (grace period for real TCP flush), `NexusSession.Receiving.cs:17` HandshakeTimeout (real handshake deadline), `SocketTransport.cs:96` ConnectionTimeout (real TCP connect), `WebSocketPipe.cs:95` close timeout (real WebSocket close-ack). Sites that remain routed through `TimeProvider`: app-logic timing decisions only — reconnect backoff (`NexusClient.cs:235`, `NexusListRelay.cs:153`), pipe back-pressure polling, all `CreateTimer` sites (ping, watchdog, health-check, rate-limit cleanup), and all elapsed-duration math (`GetTickCount64` for auth cache / LastReceived / sliding window / pool last-used). All `DisconnectDelay = 0` workarounds removed from tests; the hazard category is now eliminated rather than worked around.
