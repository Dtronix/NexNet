...
namespace PropertyStructureGenerators
{
    ...
    public class PropertyStructureHashGenerator : IIncrementalGenerator
    {
            ...
            context.RegisterSourceOutput(compilationAndSymbols, (context, source) =>
            {
                ...
                foreach (var typeSymbol in types)
                {
                    try
                    {
                        ...
                        var values = attribute.NamedArguments.First(a => a.Key == "Properties").Value.Values.Select(v => v.Value!.ToString()).ToArray();
                        var hashInt = int.Parse(attribute.NamedArguments.First(a => a.Key == "Hash").Value.Value!.ToString(), NumberStyles.Integer);
        ...
        private void ReportDiagnostic(SourceProductionContext context, Diagnostic? diagnostic, List<string>? parsed, string[]? required)
        {
            if(diagnostic != null)
