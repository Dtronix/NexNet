...
internal sealed class Arena<T> : IDisposable, IArena<T>
{
    ...
    internal Arena(ArenaOptions options, Allocator<T> allocator, int blockSizeBytes)
    {
        ...
        var blockSize = (blockSizeBytes > 0 ? blockSizeBytes : DefaultBlockSizeBytes) / Unsafe.SizeOf<T>(); // calculate the preferred block size
    ...
    /// Allocate a (possibly non-contiguous) region of memory from the arena
    /// </summary>
    public Sequence<T> Allocate(IEnumerable<T> source) => Arena.Allocate(this, source);
    ...
    private Sequence<T> SlowAllocate(long length)
    {
        ...
        var startOffset = _allocatedCurrentBlock;
    ...
    private void Trim(long retain)
    {
        ...
        while (block is not null)
        {
            ...
            if (found > retain)
            {
                ...
                var wipeBlocks = ClearAtDispose & !ClearAtReset;
