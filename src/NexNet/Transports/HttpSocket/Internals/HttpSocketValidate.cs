...
internal static partial class HttpSocketValidate
{
    /// <summary>
    ...
    internal static void ValidateSubprotocol(string subProtocol)
    {
        ...
        if (indexOfInvalidChar >= 0)
        {
            ...
            throw new ArgumentException(string.Format(Strings.net_HttpSockets_InvalidCharInProtocolString, subProtocol, invalidCharDescription), nameof(subProtocol));
        }
    }

    internal static void ValidateCloseStatus(HttpSocketCloseStatus closeStatus, string? statusDescription)
    {
        if (closeStatus == HttpSocketCloseStatus.Empty && !string.IsNullOrEmpty(statusDescription))
        {
            throw new ArgumentException(string.Format(Strings.net_HttpSockets_ReasonNotNull,
                    statusDescription,
                    HttpSocketCloseStatus.Empty),
                    nameof(statusDescription));
        }

        int closeStatusCode = (int)closeStatus;
#if TARGET_BROWSER
            // as defined in https://developer.mozilla.org/en-US/docs/Web/API/HttpSocket/close#code
            if (closeStatus != HttpSocketCloseStatus.NormalClosure && (closeStatusCode < ValidCloseStatusCodesFrom || closeStatusCode > ValidCloseStatusCodesTo))
#else
...
#endif
        {
            // CloseStatus 1006 means Aborted - this will never appear on the wire and is reflected by calling HttpSocket.Abort
            throw new ArgumentException(string.Format(Strings.net_HttpSockets_InvalidCloseStatusCode,
                    closeStatusCode),
                    nameof(closeStatus));
        }
        ...
        {
            throw new ArgumentException(string.Format(Strings.net_HttpSockets_InvalidCloseStatusDescription,
                    statusDescription,
