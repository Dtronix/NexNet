...
//using HttpSocketsTests.Compression;
...
/// </remarks>
internal sealed partial class ManagedHttpSocket : HttpSocket
{
    ...
    // We maintain the current HttpSocketState in _state.  However, we separately maintain _sentCloseFrame and _receivedCloseFrame
    ...
    private HttpSocketState _state = HttpSocketState.Open;
    ...
    private HttpSocketCloseStatus? _closeStatus;
    ...
    /// <summary>
    /// Whether the last SendAsync had <seealso cref="HttpSocketMessageFlags.DisableCompression" /> flag set.
    /// </summary>
    ...
    //private readonly HttpSocketInflater? _inflater;
    //private readonly HttpSocketDeflater? _deflater;
    ...
    internal ManagedHttpSocket(Stream stream, bool isServer, string? subprotocol, TimeSpan keepAliveInterval, TimeSpan keepAliveTimeout)
    {
        ...
        if (keepAliveInterval > TimeSpan.Zero)
        {
            ...
            {
                var wr = (WeakReference<ManagedHttpSocket>)s!;
                if (wr.TryGetTarget(out ManagedHttpSocket? thisRef))
                {
                    thisRef.HeartBeat();
                }
            }, new WeakReference<ManagedHttpSocket>(this), heartBeatIntervalMs, heartBeatIntervalMs);
        }
    }
    ...
    internal ManagedHttpSocket(Stream stream, HttpSocketCreationOptions options)
    ...
    private void DisposeCore()
    {
        ...
        if (!_disposed)
        {
            ...
            HttpSocketState state = _state;
            if (state < HttpSocketState.Aborted)
            {
    ...
    public override HttpSocketCloseStatus? CloseStatus => _closeStatus;

    public override string? CloseStatusDescription => _closeStatusDescription;

    public override HttpSocketState State => _state;
    ...
    public override Task SendAsync(ArraySegment<byte> buffer, HttpSocketMessageType messageType, bool endOfMessage, CancellationToken cancellationToken)
    {
        ...
        if (messageType != HttpSocketMessageType.Text && messageType != HttpSocketMessageType.Binary)
        {
            throw new ArgumentException(string.Format(
                     Strings.net_HttpSockets_Argument_InvalidMessageType,
                    nameof(HttpSocketMessageType.Close), nameof(SendAsync), nameof(HttpSocketMessageType.Binary), nameof(HttpSocketMessageType.Text), nameof(CloseOutputAsync)),
                nameof(messageType));
        }

        HttpSocketValidate.ValidateArraySegment(buffer, nameof(buffer));

        return SendAsync(buffer, messageType, endOfMessage ? HttpSocketMessageFlags.EndOfMessage : default, cancellationToken).AsTask();
    }
    ...
    public override ValueTask SendAsync(ReadOnlyMemory<byte> buffer, HttpSocketMessageType messageType, bool endOfMessage, CancellationToken cancellationToken) =>
        SendAsync(buffer, messageType, endOfMessage ? HttpSocketMessageFlags.EndOfMessage : default, cancellationToken);
    ...
    public override ValueTask SendAsync(ReadOnlyMemory<byte> buffer, HttpSocketMessageType messageType, HttpSocketMessageFlags messageFlags, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ThrowIfInvalidState(HttpSocketStateHelper.ValidSendStates);
        }
        catch (Exception exc)
        {
        ...
        bool endOfMessage = messageFlags.HasFlag(HttpSocketMessageFlags.EndOfMessage);
        bool disableCompression = messageFlags.HasFlag(HttpSocketMessageFlags.DisableCompression);
        ...
        if (_lastSendWasFragment)
        {
            ...
            {
                throw new ArgumentException( Strings.net_HttpSockets_Argument_MessageFlagsHasDifferentCompressionOptions, nameof(messageFlags));
            }
            opcode = MessageOpcode.Continuation;
        }
        else
        {
            opcode = messageType == HttpSocketMessageType.Binary ? MessageOpcode.Binary : MessageOpcode.Text;
        }
    ...
    public override Task<HttpSocketReceiveResult> ReceiveAsync(ArraySegment<byte> buffer, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ThrowIfInvalidState(HttpSocketStateHelper.ValidReceiveStates);

            return ReceiveAsyncPrivate<HttpSocketReceiveResult>(buffer, cancellationToken).AsTask();
        }
        catch (Exception exc)
        {
            if (NetEventSource.Log.IsEnabled()) NetEventSource.TraceException(this, exc);
            return Task.FromException<HttpSocketReceiveResult>(exc);
        }
    }
    ...
    public override ValueTask<ValueHttpSocketReceiveResult> ReceiveAsync(Memory<byte> buffer, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return ReceiveAsyncPrivate<ValueHttpSocketReceiveResult>(buffer, cancellationToken);
        }
        catch (Exception exc)
        {
            if (NetEventSource.Log.IsEnabled()) NetEventSource.TraceException(this, exc);
            return ValueTask.FromException<ValueHttpSocketReceiveResult>(exc);
        }
    }
    ...
    public override Task CloseAsync(HttpSocketCloseStatus closeStatus, string? statusDescription, CancellationToken cancellationToken)
    {
        ...
        HttpSocketValidate.ValidateCloseStatus(closeStatus, statusDescription);

        try
        {
            ThrowIfInvalidState(HttpSocketStateHelper.ValidCloseStates);
        }
        catch (Exception exc)
        {
            if (NetEventSource.Log.IsEnabled()) NetEventSource.TraceException(this, exc);
    ...
    private async Task CloseOutputAsyncCore(HttpSocketCloseStatus closeStatus, string? statusDescription, CancellationToken cancellationToken)
    {
        if (NetEventSource.Log.IsEnabled()) NetEventSource.Trace(this);

        ThrowIfInvalidState(HttpSocketStateHelper.ValidCloseOutputStates);
    ...
    private void OnAbortedCore()
    {
        ...
        HttpSocketState state = _state;
        if (state is not HttpSocketState.Closed and not HttpSocketState.Aborted)
        {
            _state = state is not HttpSocketState.None and not HttpSocketState.Connecting ?
                HttpSocketState.Aborted :
                HttpSocketState.Closed;
        }
    ...
    private ValueTask SendFrameLockAcquiredNonCancelableAsync(MessageOpcode opcode, bool endOfMessage, bool disableCompression, ReadOnlyMemory<byte> payloadBuffer)
    {
        ...
        catch (Exception exc)
        {
            ...
            return ValueTask.FromException(
                ...
                _state == HttpSocketState.Aborted ? CreateOperationCanceledException(exc) :
                new HttpSocketException(HttpSocketError.ConnectionClosedPrematurely, exc));
        }
        finally
        {
    ...
    private async ValueTask WaitForWriteTaskAsync(ValueTask writeTask, bool shouldFlush)
    {
        ...
        catch (Exception exc)
        {
            ...
            throw _state == HttpSocketState.Aborted ?
    ...
    private async ValueTask SendFrameFallbackAsync(MessageOpcode opcode, bool endOfMessage, bool disableCompression, ReadOnlyMemory<byte> payloadBuffer, Task lockTask, CancellationToken cancellationToken)
    {
        ...
        {
            int sendBytes = WriteFrameToSendBuffer(opcode, endOfMessage, disableCompression, payloadBuffer.Span);
            using (cancellationToken.Register(static s => ((ManagedHttpSocket)s!).Abort(), this))
            {
    ...
    private int WriteFrameToSendBuffer(MessageOpcode opcode, bool endOfMessage, bool disableCompression, ReadOnlySpan<byte> payloadBuffer)
    {
        ...
        if (_isServer)
        {
            ...
            headerLength = WriteHeader(opcode, _sendBuffer, payloadBuffer, endOfMessage, useMask: false, compressed: false && !disableCompression);
        }
        else
        {
            ...
            maskOffset = WriteHeader(opcode, _sendBuffer, payloadBuffer, endOfMessage, useMask: true, compressed: false && !disableCompression);
    ...
    private async ValueTask<TResult> ReceiveAsyncPrivate<TResult>(Memory<byte> payloadBuffer, CancellationToken cancellationToken)
    {
        ...
        try
        {
            registration = cancellationToken.Register(static s => ((ManagedHttpSocket)s!).Abort(), this);
            ...
            try
            {
                ...
                while (true) // in case we get control frames that should be ignored from the user's perspective
                {
                    ...
                    if (header.Processed)
                    {
                        ...
                        {
                            await CloseWithReceiveErrorAndThrowAsync(HttpSocketCloseStatus.ProtocolError, HttpSocketError.Faulted, headerErrorMessage).ConfigureAwait(false);
                        }
                        _receivedMaskOffsetOffset = 0;

                        if (NetEventSource.Log.IsEnabled())
                        {
                            NetEventSource.Trace(this, $"Next frame opcode={header.Opcode}, fin={header.Fin}, payloadLength={header.PayloadLength}");
                        }
                    }
                    ...
                    else if (header.Opcode == MessageOpcode.Close)
                    {
                        ...
                        return GetReceiveResult<TResult>(0, HttpSocketMessageType.Close, true);
                    }
                    ...
                    if (header.Processed || payloadBuffer.Length == 0)
                    {
                        ...
                        return GetReceiveResult<TResult>(
                            count: 0,
                            messageType: header.Opcode == MessageOpcode.Text ? HttpSocketMessageType.Text : HttpSocketMessageType.Binary,
                    ...
                    if (header.PayloadLength > 0)
                    {
                        ...
                        int limit = (int)Math.Min(payloadBuffer.Length, header.PayloadLength);
                        ...
                        if (totalBytesReceived < limit)
                        {
                            ...
                            Memory<byte> readBuffer = payloadBuffer.Slice(totalBytesReceived, bytesToRead);
                    ...
                    {
                        await CloseWithReceiveErrorAndThrowAsync(HttpSocketCloseStatus.InvalidPayloadData, HttpSocketError.Faulted).ConfigureAwait(false);
                    }
                    ...
                    return GetReceiveResult<TResult>(
                        totalBytesReceived,
                        header.Opcode == MessageOpcode.Text ? HttpSocketMessageType.Text : HttpSocketMessageType.Binary,
                        header.EndOfMessage);
                }
            }
            finally
        ...
        catch (Exception exc)
        {
            ...
            if (_state == HttpSocketState.Aborted)
            {
                Exception inner = exc;
                ...
                throw new OperationCanceledException(nameof(HttpSocketState.Aborted), inner);
            }
            OnAborted();
            ...
            throw new HttpSocketException(HttpSocketError.ConnectionClosedPrematurely, exc);
        }
        finally
        {
    ...
    /// <summary>
    /// Returns either <see cref="ValueHttpSocketReceiveResult"/> or <see cref="HttpSocketReceiveResult"/>.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private TResult GetReceiveResult<TResult>(int count, HttpSocketMessageType messageType, bool endOfMessage)
    {
        if (typeof(TResult) == typeof(ValueHttpSocketReceiveResult))
        {
            ...
            // bytecode that returns the ValueHttpSocketReceiveResult directly.
            return (TResult)(object)new ValueHttpSocketReceiveResult(count, messageType, endOfMessage);
        }

        return (TResult)(object)new HttpSocketReceiveResult(count, messageType, endOfMessage, _closeStatus, _closeStatusDescription);
    }
    ...
    private async ValueTask HandleReceivedCloseAsync(MessageHeader header, CancellationToken cancellationToken)
    {
        lock (StateUpdateLock)
        {
            ...
            HttpSocketState state = _state;
            if (_sentCloseFrame && state < HttpSocketState.Closed)
            {
                _state = HttpSocketState.Closed;
            }
            else if (state < HttpSocketState.CloseReceived)
            {
                _state = HttpSocketState.CloseReceived;
            }

            if (NetEventSource.Log.IsEnabled()) NetEventSource.Trace(this, $"State transition from {state} to {_state}");
        }

        HttpSocketCloseStatus closeStatus = HttpSocketCloseStatus.NormalClosure;
        ...
        if (header.PayloadLength == 1)
        {
            ...
            await CloseWithReceiveErrorAndThrowAsync(HttpSocketCloseStatus.ProtocolError, HttpSocketError.Faulted).ConfigureAwait(false);
        }
        else if (header.PayloadLength >= 2)
        {
            ...
            closeStatus = (HttpSocketCloseStatus)BinaryPrimitives.ReadUInt16BigEndian(_receiveBuffer.Span.Slice(_receiveBufferOffset));
            ...
            if (header.PayloadLength > 2)
            {
                ...
                {
                    await CloseWithReceiveErrorAndThrowAsync(HttpSocketCloseStatus.ProtocolError, HttpSocketError.Faulted, innerException: exc).ConfigureAwait(false);
                }
            }
...
#pragma warning restore CA2016
    ...
    private static bool IsValidCloseStatus(HttpSocketCloseStatus closeStatus)
    {
        // 0-999: "not used"
        ...
        if (closeStatus < (HttpSocketCloseStatus)1000 || closeStatus >= (HttpSocketCloseStatus)5000)
        {
            return false;
        }

        if (closeStatus >= (HttpSocketCloseStatus)3000)
        {
            return true;
        }
        ...
        {
            case HttpSocketCloseStatus.EndpointUnavailable:
            case HttpSocketCloseStatus.InternalServerError:
            case HttpSocketCloseStatus.InvalidMessageType:
            case HttpSocketCloseStatus.InvalidPayloadData:
            case HttpSocketCloseStatus.MandatoryExtension:
            case HttpSocketCloseStatus.MessageTooBig:
            case HttpSocketCloseStatus.NormalClosure:
            case HttpSocketCloseStatus.PolicyViolation:
            case HttpSocketCloseStatus.ProtocolError:
            case (HttpSocketCloseStatus)1012: // ServiceRestart
            case (HttpSocketCloseStatus)1013: // TryAgainLater
            case (HttpSocketCloseStatus)1014: // BadGateway
                return true;
    ...
    private async ValueTask CloseWithReceiveErrorAndThrowAsync(
        HttpSocketCloseStatus closeStatus, HttpSocketError error, string? errorMessage = null, Exception? innerException = null)
    {
        ...
        throw errorMessage != null ?
            new HttpSocketException(error, errorMessage, innerException) :
            new HttpSocketException(error, innerException);
    }
    ...
    private string? TryParseMessageHeaderFromReceiveBuffer(out MessageHeader resultHeader)
    {
        ...
        if (reservedSet)
        {
            ...
            return Strings.net_HttpSockets_ReservedBitsSet;
        }
        ...
        if (header.PayloadLength < 0)
        {
            ...
            resultHeader = default;
            return Strings.net_HttpSockets_InvalidPayloadLength;
        }

        //if (header.Compressed && _inflater is null)
        //{
        //    resultHeader = default;
        //    return Strings.net_HttpSockets_PerMessageCompressedFlagWhenNotEnabled;
        //}
        ...
        if (masked)
        {
            ...
            {
                resultHeader = default;
                return Strings.net_HttpSockets_ClientReceivedMaskedFrame;
            }
        ...
        switch (header.Opcode)
        {
            case MessageOpcode.Continuation:
                if (_lastReceiveHeader.Fin)
                {
                    ...
                    resultHeader = default;
                    return Strings.net_HttpSockets_ContinuationFromFinalFrame;
                }
                //if (header.Compressed)
                //{
                //    // Must not mark continuations as compressed
                //    resultHeader = default;
                //    return Strings.net_HttpSockets_PerMessageCompressedFlagInContinuation;
                //}
            ...
            case MessageOpcode.Text:
                if (!_lastReceiveHeader.Fin)
                {
                    ...
                    resultHeader = default;
                    return  Strings.net_HttpSockets_NonContinuationAfterNonFinalFrame;
                }
                break;
            ...
            case MessageOpcode.Pong:
                if (header.PayloadLength > MaxControlPayloadLength || !header.Fin)
                {
                    ...
                    resultHeader = default;
                    return  Strings.net_HttpSockets_InvalidControlMessage;
                }
                break;
            ...
            default:
                // Unknown opcode
                resultHeader = default;
                return string.Format( Strings.net_HttpSockets_UnknownOpcode, header.Opcode);
        }
    ...
    /// <param name="cancellationToken">The CancellationToken to use to cancel the websocket.</param>
    private async Task CloseAsyncPrivate(HttpSocketCloseStatus closeStatus, string? statusDescription, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            Debug.Assert(
                State == HttpSocketState.CloseSent ||
                State == HttpSocketState.Closed ||
                State == HttpSocketState.Aborted,
            ...
            // wait for a close frame as per RFC 6455 Section 7.1.7 "Fail the HttpSocket Connection".
            if (State == HttpSocketState.CloseSent)
            {
                ...
                try
                {
                    ...
                    while (!_receivedCloseFrame)
                    {
                        ...
                        ValueTask<ValueHttpSocketReceiveResult> receiveTask = default;
                        try
                        {
                            await _receiveMutex.EnterAsync(cancellationToken).ConfigureAwait(false);
                            if (NetEventSource.Log.IsEnabled()) NetEventSource.MutexEntered(_receiveMutex);
                            ...
                            try
                            {
                                if (!_receivedCloseFrame)
                                {
                                    receiveTask = ReceiveAsyncPrivate<ValueHttpSocketReceiveResult>(closeBuffer, cancellationToken);
                                }
                            }
                            finally
                            {
    ...
    /// <param name="cancellationToken">The CancellationToken to use to cancel the websocket.</param>
    private async ValueTask SendCloseFrameAsync(HttpSocketCloseStatus closeStatus, string? closeStatusDescription, CancellationToken cancellationToken)
    {
        ...
        lock (StateUpdateLock)
        {
            ...
            HttpSocketState state = _state;
            if (_receivedCloseFrame && state < HttpSocketState.Closed)
            {
                _state = HttpSocketState.Closed;
            }
            else if (state < HttpSocketState.CloseSent)
            {
                _state = HttpSocketState.CloseSent;
            }
    ...
    private static void ThrowOperationInProgress(string? methodName) => throw new InvalidOperationException(string.Format( Strings.net_HttpSockets_AlreadyOneOutstandingOperation, methodName));
    ...
    private void ThrowIfDisposed() => ThrowIfInvalidState(validStates: ManagedHttpSocketStates.All);

    private void ThrowIfInvalidState(ManagedHttpSocketStates validStates)
    {
        ...
        HttpSocketStateHelper.ThrowIfInvalidState(state, disposed, keepAliveException, validStates);
    }

    // From https://github.com/aspnet/HttpSocketsTests/blob/aa63e27fce2e9202698053620679a9a1059b501e/src/Microsoft.AspNetCore.HttpSocketsTests.Protocol/Utilities.cs#L75
