...
internal sealed partial class ManagedHttpSocket : HttpSocket
{
    ...
    private ValueTask TrySendKeepAliveFrameAsync(MessageOpcode opcode, ReadOnlyMemory<byte> payload = default)
    {
        ...
        if (!HttpSocketStateHelper.IsValidSendState(_state))
        {
                ...
                if (_keepAlivePingState.PingSent)
                {
                    if (now > _keepAlivePingState.PingTimeoutTimestamp)
                    {
                        ...
                        Exception exc = ExceptionDispatchInfo.SetCurrentStackTrace(
                            new HttpSocketException(HttpSocketError.Faulted, Strings.net_HttpSockets_KeepAlivePingTimeout));
    ...
    private sealed class KeepAlivePingState
    {
        ...
        private readonly ManagedHttpSocket _parent;
        ...
        public KeepAlivePingState(TimeSpan keepAliveInterval, TimeSpan keepAliveTimeout, ManagedHttpSocket parent)
        {
        ...
        internal void OnKeepAliveFaultedCore(Exception exc)
        {
            ...
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Trace(this, $"HttpSocket already disposed, skipping...");
                return;
            }

            if (_parent.State is HttpSocketState.Closed)
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Trace(this, $"HttpSocket is already closed, skipping...");
            ...
            if (_parent.State is HttpSocketState.Aborted)
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Trace(this, $"HttpSocket is already aborted, skipping...");
