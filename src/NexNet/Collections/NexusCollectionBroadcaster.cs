...
{
    private readonly SnapshotList<NexusCollectionClient> _connectedClients;
    private readonly INexusLogger? _logger;
    private readonly Channel<NexusBroadcastMessageWrapper> _messageBroadcastChannel;
    
    private bool _isRunning;
    protected NexusCollectionBroadcaster(INexusLogger? logger)
    {
        _logger = logger?.CreateLogger("Broadcast");
        _connectedClients = new SnapshotList<NexusCollectionClient>(64);
    ...
    public void AddClientAsync(NexusCollectionClient client)
    {
        if(!_isRunning)
            throw new InvalidOperationException("Broadcaster is not running.");
        
        // Add in the completion removal for execution later.
        _logger?.LogTrace($"S{client.Session.Id} Starting collection message writer.");
        
        // Start the listener on the channel to handle sending updates to the client.
        Task.Factory.StartNew(static async state =>
        {
            var client = (NexusCollectionClient)(state!);
            NexusBroadcastMessageWrapper? wrapper = null;
            try
            {
                await foreach (NexusBroadcastMessageWrapper messageWrapper in client.MessageSender.Reader.ReadAllAsync(client.CompletionToken).ConfigureAwait(false))
                {
                    ...
                    await client.Writer!.WriteAsync(message, client.CompletionToken)
                        .ConfigureAwait(false);
                }
            }
            catch (Exception e)
            {
                client.Session.Logger?.LogInfo(e,
                    $"S{client.Session.Id} Could not send collection broadcast message to session.");
    ...
    public ValueTask BroadcastAsync(INexusCollectionMessage message, NexusCollectionClient? sourceClient)
    {
                ...
                try
                {
                    await foreach (var broadcastMessage in broadcaster._messageBroadcastChannel.Reader.ReadAllAsync(ct).ConfigureAwait(false))
                    {
                        ...
                        foreach (var client in broadcaster._connectedClients)
                        {
                            try
                            {
                                if (!client.MessageSender.Writer.TryWrite(broadcastMessage))
                                {
                                ...
                                {
                                    broadcaster._logger?.LogTrace(
                                        $"S{client.Session.Id} Sent to client collection");
                                }
                            }
                            catch (Exception e)
                            {
                                broadcaster._logger?.LogInfo(e,
                                    $"S{client.Session.Id} Exception while sending to collection. Removing from broadcast.");
...
internal class NexusBroadcastMessageWrapper
{
    ...
    public NexusCollectionClient? SourceClient { get; }
    ...
    public NexusBroadcastMessageWrapper(NexusCollectionClient? sourceClient, INexusCollectionMessage message)
    {
