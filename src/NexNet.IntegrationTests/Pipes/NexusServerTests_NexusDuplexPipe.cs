...
internal class NexusServerTests_NexusDuplexPipe : BasePipeTests
{
    ...
    [TestCase(Type.WebSocket)]
    [TestCase(Type.HttpSocket)]
    public async Task Server_ChannelReader_IAsyncEnumerable(Type type)
    {
        var (_, sNexus, _, cNexus, _) = await Setup(type);
        ...
        cNexus.ClientTaskValueWithDuplexPipeEvent = async (nexus, pipe) =>
        {
            ...
            int counter = 0;
            await foreach (var message in await pipe.GetChannelReader<ComplexMessage>())
            {
        ...
        _ = Task.Run(async () =>
        {
            await foreach (var message in await pipe.GetUnmanagedChannelReader<int>())
            {
