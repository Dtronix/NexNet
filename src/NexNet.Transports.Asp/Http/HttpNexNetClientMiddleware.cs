...
/// <summary>
/// Enables accepting HttpSocket requests by adding a <see cref="IHttpHttpSocketFeature"/>
/// to the <see cref="HttpContext"/> if the request is a valid HttpSocket request.
/// </summary>
public partial class HttpSocketMiddleware
{
    ...
    public HttpSocketMiddleware(RequestDelegate next, IOptions<HttpSocketOptions> options, ILoggerFactory loggerFactory)
    {
    ...
    /// sets the <see cref="IHttpHttpSocketFeature"/> on the <see cref="HttpContext.Features"/>.
    /// </summary>
    ...
    public Task Invoke(HttpContext context)
    {
        ...
        if ((upgradeFeature != null || connectFeature != null) && context.Features.Get<IHttpHttpSocketFeature>() == null)
        {
            var webSocketFeature = new HttpSocketHandshake(context, upgradeFeature, connectFeature, _options, _logger);
            context.Features.Set<IHttpHttpSocketFeature>(webSocketFeature);
    ...
    private sealed class HttpSocketHandshake : IHttpHttpSocketFeature
    {
        ...
        public bool IsHttpSocketRequest
        {
            get
            {
                if (_isHttpSocketRequest == null)
                {
                    ...
                    {
                        _isH2HttpSocket = CheckSupportedHttpSocketRequestH2(_context.Request.Method, _connectFeature.Protocol, _context.Request.Headers);
        ...
        public async Task<HttpSocket> AcceptAsync(HttpSocketAcceptContext acceptContext)
        {
...
#pragma warning disable CS0618 // Type or member is obsolete
            if (acceptContext is ExtendedHttpSocketAcceptContext advancedAcceptContext)
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            HandshakeHelpers.GenerateResponseHeaders(!_isH2HttpSocket, _context.Request.Headers, subProtocol, _context.Response.Headers);

            HttpSocketDeflateOptions? deflateOptions = null;
            if (enableCompression)
            {
                var ext = _context.Request.Headers.SecHttpSocketExtensions;
                if (ext.Count != 0)
                {
                    // loop over each extension offer, extensions can have multiple offers, we can accept any
                    foreach (var extension in _context.Request.Headers.GetCommaSeparatedValues(HeaderNames.SecHttpSocketExtensions))
                    {
                        if (extension.AsSpan().TrimStart().StartsWith("permessage-deflate", StringComparison.Ordinal))
                        {
                            if (HandshakeHelpers.ParseDeflateOptions(extension.AsSpan().TrimStart(), serverContextTakeover, serverMaxWindowBits, out var parsedOptions, out var response))
                            {
                                ...
                                _context.Response.Headers.SecHttpSocketExtensions = response;
                                break;
                            }
                        }
                    }
            ...
            var wrappedSocket = HttpSocket.CreateFromStream(abortStream, new HttpSocketCreationOptions()
            {
                IsServer = true,
            ...
            abortStream.HttpSocket = wrappedSocket;
            return wrappedSocket;
        }

        public static bool CheckSupportedHttpSocketRequest(string method, IHeaderDictionary requestHeaders)
        {
            ...
            if (!CheckHttpSocketVersion(requestHeaders))
            {
                return false;
            }
            ...
            {
                if (string.Equals(value, Constants.Headers.UpgradeHttpSocket, StringComparison.OrdinalIgnoreCase))
                {
                    // HttpSockets are long lived; so if the header values are valid we switch them out for the interned versions.
                    if (values.Length == 1)
                    {
                        requestHeaders.Upgrade = Constants.Headers.UpgradeHttpSocket;
                    }
                    foundHeader = true;
                    break;
                }
            ...
            return HandshakeHelpers.IsRequestKeyValid(requestHeaders.SecHttpSocketKey.ToString());
        }
        ...
        public static bool CheckSupportedHttpSocketRequestH2(string method, string? protocol, IHeaderDictionary requestHeaders)
        {
            return HttpMethods.IsConnect(method)
                && string.Equals(protocol, Constants.Headers.UpgradeHttpSocket, StringComparison.OrdinalIgnoreCase)
                && CheckHttpSocketVersion(requestHeaders);
        }

        public static bool CheckHttpSocketVersion(IHeaderDictionary requestHeaders)
        {
            var values = requestHeaders.GetCommaSeparatedValues(HeaderNames.SecHttpSocketVersion);
            foreach (var value in values)
            {
                if (string.Equals(value, Constants.Headers.SupportedVersion, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (values.Length == 1)
                    {
                        requestHeaders.SecHttpSocketVersion = Constants.Headers.SupportedVersion;
                    }
                    return true;
                }
            }
    ...
    {
        [LoggerMessage(1, LogLevel.Debug, "HttpSocket compression negotiation accepted with values '{CompressionResponse}'.", EventName = "CompressionAccepted")]
